...
from app.utils.MVSDK import *
from app.utils.ImageConvert import *
...
class CameraAPI:
    ...
    def discover_cameras(self, system):
        ...
        if nRet != 0:
            logger.error("Не удалось обнаружить камеры!")
    ...
    def capture_frame_bytes(self):
        ...
        buf = self._get_image_as_bmp(convertParams, frameBuff)
    ...
    def _get_image_as_bmp(self, convertParams, frameBuff):
        """
        Возвращает изображение в формате BMP в виде байтов.


        :param convertParams: Параметры для конвертации.
        :param frameBuff: Буфер с данными изображения.
        :return: Байты изображения в формате BMP или None при ошибке.
        """
        logger.info("Получение изображения в формате BMP")
        ...
        try:
            # Создаем буфер для BMP данных
            bmp_data = bytearray()
            bmp_data.extend(struct.pack('H', bmpFileHeader.bfType))
            bmp_data.extend(struct.pack('I', bmpFileHeader.bfSize))
            bmp_data.extend(struct.pack('H', bmpFileHeader.bfReserved1))
            bmp_data.extend(struct.pack('H', bmpFileHeader.bfReserved2))
            bmp_data.extend(struct.pack('I', bmpFileHeader.bfOffBits))
            bmp_data.extend(struct.pack('I', bmpInfoHeader.biSize))
            bmp_data.extend(struct.pack('i', bmpInfoHeader.biWidth))
            bmp_data.extend(struct.pack('i', bmpInfoHeader.biHeight))
            bmp_data.extend(struct.pack('H', bmpInfoHeader.biPlanes))
            bmp_data.extend(struct.pack('H', bmpInfoHeader.biBitCount))
            bmp_data.extend(struct.pack('I', bmpInfoHeader.biCompression))
            bmp_data.extend(struct.pack('I', bmpInfoHeader.biSizeImage))
            bmp_data.extend(struct.pack('i', bmpInfoHeader.biXPelsPerMeter))
            bmp_data.extend(struct.pack('i', bmpInfoHeader.biYPelsPerMeter))
            bmp_data.extend(struct.pack('I', bmpInfoHeader.biClrUsed))
            bmp_data.extend(struct.pack('I', bmpInfoHeader.biClrImportant))
            
            if convertParams.pixelForamt == EPixelType.gvspPixelMono8:
                for i in range(256):
                    bmp_data.extend(struct.pack('B', rgbQuad[i].rgbBlue))
                    bmp_data.extend(struct.pack('B', rgbQuad[i].rgbGreen))
                    bmp_data.extend(struct.pack('B', rgbQuad[i].rgbRed))
                    bmp_data.extend(struct.pack('B', rgbQuad[i].rgbReserved))
                bmp_data.extend(frameBuff)
            else:
                bmp_data.extend(rgbBuff)
        except Exception as e:
            logger.error(f"Ошибка формирования BMP: {e}")
            return None


        logger.info("Изображение сформировано в формате BMP")
        return bmp_data
